...
`/api/v1/effects/remove-background` recibe `{ "input_filename": "hero.png", "output_filename": "hero-cutout.png", "background_color": "#142850", "tolerance": 12, "connected_only": true }`. Convierte a transparencia únicamente los píxeles del color objetivo conectados al borde cuando `connected_only` está activo; con `false` elimina todas las coincidencias. Mantiene dimensiones, frames, colores RGB y archivo fuente.

`/api/v1/effects/cleanup` recibe `{ "input_filename": "hero.png", "output_filename": "hero-clean.png", "min_neighbors": 1, "iterations": 1 }`. Elimina píxeles opacos con menos vecinos de los indicados usando conectividad de 8 direcciones, permite repetir la pasada de forma acotada y conserva dimensiones, frames y el archivo fuente.

`/api/v1/effects/glow` recibe `{ "input_filename": "hero.png", "output_filename": "hero-glow.png", "color": "#FFD166", "radius": 2, "opacity": 0.8 }`. Genera un halo radial determinista solo en píxeles transparentes cercanos a la silueta y mantiene intactos los píxeles originales y sus frames.

`/api/v1/effects/rim-light` recibe `{ "input_filename": "hero.png", "output_filename": "hero-rim.png", "color": "#FFD166", "direction": "north_east", "strength": 0.75 }`. Mezcla el color únicamente en píxeles opacos con vecino transparente hacia la dirección seleccionada; admite ocho direcciones, conserva alpha y no modifica la fuente.

`/api/v1/effects/ambient-occlusion` recibe `{ "input_filename": "hero.png", "output_filename": "hero-ambient.png", "color": "#000000", "radius": 1, "strength": 0.6 }`. Calcula una oclusión reproducible a partir de vecinos transparentes/opacos, oscurece cavidades y bordes internos, conserva alpha, frames y la fuente.

`/api/v1/effects/specular-highlight` recibe `{ "input_filename": "hero.png", "output_filename": "hero-specular.png", "color": "#FFFFFF", "direction": "north", "radius": 2, "strength": 0.8 }`. Propaga el brillo desde los bordes transparentes hacia el interior de la silueta con caída por distancia; es determinista, conserva alpha y resulta útil para superficies metálicas, agua, cristal y efectos mágicos.

`/api/v1/effects/color-ramp` recibe `{ "input_filename": "hero.png", "output_filename": "hero-ramp.png", "shadow_color": "#101020", "mid_color": "#6080A0", "highlight_color": "#FFFFFF", "shadow_threshold": 0.3, "highlight_threshold": 0.7 }`. Clasifica la luminancia de cada píxel opaco en tres bandas, conserva la alpha original y permite aplicar una paleta de sombreado consistente a personajes, criaturas y escenarios.
