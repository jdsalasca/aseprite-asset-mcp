...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 28 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "layer-management",
  currentSlice: "layer-management-tools",
  lastCompletedSlice: "convolution-and-dither-tools",
  nextSlice: "legacy-fx-compatibility",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
