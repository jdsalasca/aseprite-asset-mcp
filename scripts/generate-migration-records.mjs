...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 32 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "pixel-read",
  currentSlice: "pixel-read-tools",
  lastCompletedSlice: "palette-tool-expansion",
  nextSlice: "selection-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
