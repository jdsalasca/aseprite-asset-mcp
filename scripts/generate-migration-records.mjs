...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 20 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "layer-tag-export",
  currentSlice: "layer-and-tag-export",
  lastCompletedSlice: "ellipse-and-basic-export",
  nextSlice: "import-and-cel-processing",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
