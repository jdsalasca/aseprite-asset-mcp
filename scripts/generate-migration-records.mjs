...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 29 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "legacy-fx",
  currentSlice: "legacy-fx-compatibility",
  lastCompletedSlice: "layer-management-tools",
  nextSlice: "legacy-hsl-adjustment",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
