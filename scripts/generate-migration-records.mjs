...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 19 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "ellipse-export",
  currentSlice: "ellipse-and-basic-export",
  lastCompletedSlice: "polygon-path-gradient",
  nextSlice: "layer-and-tag-export-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
