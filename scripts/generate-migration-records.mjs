...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 27 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "convolution-dither",
  currentSlice: "convolution-and-dither-tools",
  lastCompletedSlice: "native-color-effects",
  nextSlice: "layer-management-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
