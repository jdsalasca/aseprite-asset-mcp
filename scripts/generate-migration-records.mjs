...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 41 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  continue: true,
  currentPhase: "slices",
  currentSlice: "slice-tools",
  lastCompletedSlice: "tilemap-tools",
  nextSlice: "quality-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
