...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 30 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "legacy-hsl",
  currentSlice: "legacy-hsl-adjustment",
  lastCompletedSlice: "legacy-fx-compatibility",
  nextSlice: "palette-tool-expansion",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
