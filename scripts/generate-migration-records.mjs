...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 23 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "animation-controls",
  currentSlice: "delete-and-onion-controls",
  lastCompletedSlice: "position-and-propagation",
  nextSlice: "analysis-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
