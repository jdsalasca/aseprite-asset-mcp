...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 18 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "polygon-path-gradient",
  currentSlice: "polygon-path-gradient",
  lastCompletedSlice: "layer-frame-drawing",
  nextSlice: "ellipse-and-export-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
