...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 25 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "color-analysis",
  currentSlice: "opacity-and-palette-analysis",
  lastCompletedSlice: "visual-frame-analysis",
  nextSlice: "native-color-effects",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
