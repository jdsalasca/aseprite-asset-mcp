...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 17 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "layer-frame-drawing",
  currentSlice: "targeted-drawing",
  lastCompletedSlice: "drawing-primitives",
  nextSlice: "polygon-path-gradient-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
