...
const tools = upstreamTools.map((name) => ({
  ...
  domain: domain(name),
  tests: implemented.has(name) ? ["tests-ts/gateway-validation.test.ts", "tests-ts/integration/real-aseprite.test.ts", "tests-ts/mcp-server.test.ts"] : [],
  dependencies: [],
  commit: null,
  evidence: [],
}));
...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 24 },
  tools,
  nodes: [
    ...
    { id: "e2e-tests", type: "e2e-tests", status: "verified" },
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "analysis-tools",
  currentSlice: "visual-frame-analysis",
  lastCompletedSlice: "delete-and-onion-controls",
  nextSlice: "cel-opacity-and-palette-analysis",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
