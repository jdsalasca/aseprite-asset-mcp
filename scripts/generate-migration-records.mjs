...
writeFileSync(join(recordsDir, "migration.graph.json"), JSON.stringify({
  ...
  local: { branch, worktree, baseCommit: localCommit, typescriptToolCount: typescriptTools.length, testCount: 31 },
  tools,
  nodes: [
...
writeFileSync(join(root, ".harness-moon", "continuation-state.json"), JSON.stringify({
  ...
  goal: "complete-typescript-migration",
  continue: true,
  currentPhase: "palette-expansion",
  currentSlice: "palette-tool-expansion",
  lastCompletedSlice: "legacy-hsl-adjustment",
  nextSlice: "pixel-read-tools",
  blocked: false,
  retryCount: 0,
  lastEvidence: [
