...
test("scene effect stack delegates selected effects once, infers particle size, and preserves order", async () => {
  const calls: string[] = [];
  const result = await service(calls).generateSceneEffectStack({ inputFilename: "scene.png", outputPrefix: "out/scene", effects: ["material_texture", "rain", "wind_sway", "particles", "water_caustics"], frames: 6, seed: 33, material: "earth", format: "gif" });
  assert.equal(result.ok, true);
  assert.deepEqual(calls, ["material", "rain", "wind", "particles:12x10", "caustics"]);
  ...
  assert.deepEqual(payload.artifacts.map((artifact) => artifact.effect), ["material_texture", "rain", "wind_sway", "particles", "water_caustics"]);
