...
function fakeUseCases(): AssetRestUseCases {
  const effects: SpriteEffectsGateway = { applyPixelOutline: async () => result("apply_pixel_outline"), removeBackground: async () => result("remove_background"), cleanupIsolatedPixels: async () => result("cleanup_isolated_pixels"), generateSpriteGlow: async () => result("generate_sprite_glow"), applySpriteRimLight: async () => result("apply_sprite_rim_light"), applySpriteAmbientOcclusion: async () => result("apply_sprite_ambient_occlusion"), applyColorGrade: async () => result("apply_color_grade"), generateSpriteShadow: async () => result("generate_sprite_shadow"), generateParticleBurst: async () => result("generate_particle_burst"), generateNormalMap: async () => result("generate_normal_map"), generateRainOverlay: async () => result("generate_rain_overlay"), generateMotionPack: async () => result("generate_motion_pack"), generateSeamlessTexture: async () => result("generate_seamless_texture"), generateWaterReflection: async () => result("generate_water_reflection"), generateWaterCaustics: async () => result("generate_water_caustics"), generateDayNightCycle: async () => result("generate_day_night_cycle") };
  return {
