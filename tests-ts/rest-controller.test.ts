...
function fakeUseCases(): AssetRestUseCases {
  const effects: SpriteEffectsGateway = { applyPixelOutline: async () => result("apply_pixel_outline"), removeBackground: async () => result("remove_background"), cleanupIsolatedPixels: async () => result("cleanup_isolated_pixels"), generateSpriteGlow: async () => result("generate_sprite_glow"), generateSpriteSilhouette: async () => result("generate_sprite_silhouette"), applySpriteRimLight: async () => result("apply_sprite_rim_light"), applySpriteAmbientOcclusion: async () => result("apply_sprite_ambient_occlusion"), applySpriteSpecularHighlight: async () => result("apply_sprite_specular_highlight"), applySpriteColorRamp: async () => result("apply_sprite_color_ramp"), applySpriteGrain: async () => result("apply_sprite_grain"), applySpriteDither: async () => result("apply_sprite_dither"), applySpriteColorTemperature: async () => result("apply_sprite_color_temperature"), applyColorGrade: async () => result("apply_color_grade"), generateSpriteShadow: async () => result("generate_sprite_shadow"), generateParticleBurst: async () => result("generate_particle_burst"), generateNormalMap: async () => result("generate_normal_map"), generateRainOverlay: async () => result("generate_rain_overlay"), generateFogOverlay: async () => result("generate_fog_overlay"), generateSnowOverlay: async () => result("generate_snow_overlay"), generateSmokeOverlay: async () => result("generate_smoke_overlay"), generateFireOverlay: async () => result("generate_fire_overlay"), generateLightningOverlay: async () => result("generate_lightning_overlay"), generateWaveOverlay: async () => result("generate_wave_overlay"), generateWaterSpray: async () => result("generate_water_spray"), generateDustOverlay: async () => result("generate_dust_overlay"), generateLeafFallOverlay: async () => result("generate_leaf_fall_overlay"), generateWaterRippleOverlay: async () => result("generate_water_ripple_overlay"), generateMotionPack: async () => result("generate_motion_pack"), generateWindSway: async () => result("generate_wind_sway"), generateSeamlessTexture: async () => result("generate_seamless_texture"), generateWaterReflection: async () => result("generate_water_reflection"), generateWaterCaustics: async () => result("generate_water_caustics"), generateDayNightCycle: async () => result("generate_day_night_cycle") };
  return {
...
test("REST controller exposes the same recipe and effect application services", async () => {
  ...
  try {
    ...
    const ambient = await fetch(rest.url + "/api/v1/effects/ambient-occlusion", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "ambient.png", color: "#000000", radius: 1, strength: 0.6 }) });
    assert.equal(ambient.status, 200);
    assert.equal((await ambient.json()).data.operation, "apply_sprite_ambient_occlusion");
    const specular = await fetch(rest.url + "/api/v1/effects/specular-highlight", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "specular.png", color: "#FFFFFF", direction: "north", radius: 2, strength: 0.8 }) });
    assert.equal(specular.status, 200);
    assert.equal((await specular.json()).data.operation, "apply_sprite_specular_highlight");
    const colorRamp = await fetch(rest.url + "/api/v1/effects/color-ramp", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "ramp.png", shadow_color: "#101020", mid_color: "#6080A0", highlight_color: "#FFFFFF", shadow_threshold: 0.3, highlight_threshold: 0.7 }) });
    assert.equal(colorRamp.status, 200);
    assert.equal((await colorRamp.json()).data.operation, "apply_sprite_color_ramp");
    const grain = await fetch(rest.url + "/api/v1/effects/grain", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "grain.png", seed: 17, intensity: 0.8, scale: 2 }) });
    assert.equal(grain.status, 200);
    assert.equal((await grain.json()).data.operation, "apply_sprite_grain");
    const dither = await fetch(rest.url + "/api/v1/effects/dither", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "dither.png", dark_color: "#202030", light_color: "#F0E8C8", strength: 1, scale: 1 }) });
    assert.equal(dither.status, 200);
    assert.equal((await dither.json()).data.operation, "apply_sprite_dither");
    const temperature = await fetch(rest.url + "/api/v1/effects/color-temperature", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "warm.png", temperature: 1, intensity: 0.7 }) });
    assert.equal(temperature.status, 200);
    assert.equal((await temperature.json()).data.operation, "apply_sprite_color_temperature");
    const silhouette = await fetch(rest.url + "/api/v1/effects/silhouette", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "input.png", output_filename: "silhouette.png", color: "#08111F", opacity: 0.6 }) });
    assert.equal(silhouette.status, 200);
    assert.equal((await silhouette.json()).data.operation, "generate_sprite_silhouette");
    const wind = await fetch(rest.url + "/api/v1/effects/wind-sway", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "tree.png", output_filename: "tree-wind.gif", frames: 6, seed: 19, amplitude: 2, direction: "right", delay_ms: 75 }) });
    assert.equal(wind.status, 200);
    assert.equal((await wind.json()).data.operation, "generate_wind_sway");
  ...
  const fog = await fetch(`${rest.url}/api/v1/effects/fog`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-fog.gif", seed: 11, density: 0.65, drift: -0.2, color: "#DDEBFF", frames: 6, format: "gif" }) });
  assert.equal(fog.status, 200);
  assert.equal((await fog.json()).data.operation, "generate_fog_overlay");
  const snow = await fetch(`${rest.url}/api/v1/effects/snow`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-snow.gif", seed: 31, density: 0.8, wind: -0.25, color: "#F5FBFF", frames: 6, format: "gif" }) });
  assert.equal(snow.status, 200);
  assert.equal((await snow.json()).data.operation, "generate_snow_overlay");
  const smoke = await fetch(`${rest.url}/api/v1/effects/smoke`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-smoke.gif", seed: 41, density: 0.75, drift: 0.2, rise: 0.8, color: "#8A91A8", frames: 6, format: "gif" }) });
  assert.equal(smoke.status, 200);
  assert.equal((await smoke.json()).data.operation, "generate_smoke_overlay");
  const fire = await fetch(`${rest.url}/api/v1/effects/fire`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-fire.gif", seed: 53, intensity: 0.82, flicker: 0.7, color: "#FFD65A", frames: 6, format: "gif" }) });
  assert.equal(fire.status, 200);
  assert.equal((await fire.json()).data.operation, "generate_fire_overlay");
  const lightning = await fetch(`${rest.url}/api/v1/effects/lightning`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-lightning.gif", seed: 67, intensity: 0.72, flash: 0.8, color: "#D8F3FF", frames: 6, format: "gif" }) });
  assert.equal(lightning.status, 200);
  assert.equal((await lightning.json()).data.operation, "generate_lightning_overlay");
  const wave = await fetch(`${rest.url}/api/v1/effects/waves`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-waves.gif", seed: 79, density: 0.78, amplitude: 3, color: "#E6FAFF", frames: 6, format: "gif" }) });
  assert.equal(wave.status, 200);
  assert.equal((await wave.json()).data.operation, "generate_wave_overlay");
  const spray = await fetch(`${rest.url}/api/v1/effects/water-spray`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-spray.gif", seed: 83, density: 0.7, drift: 0.4, color: "#F4FDFF", frames: 5, format: "gif" }) });
  assert.equal(spray.status, 200); assert.equal((await spray.json()).data.operation, "generate_water_spray");
  const dust = await fetch(`${rest.url}/api/v1/effects/dust`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-dust.gif", seed: 97, density: 0.72, drift: 0.25, rise: 0.5, color: "#C79A68", frames: 5, format: "gif" }) });
  assert.equal(dust.status, 200); assert.equal((await dust.json()).data.operation, "generate_dust_overlay");
  const leafFall = await fetch(`${rest.url}/api/v1/effects/leaf-fall`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-leaves.gif", seed: 103, density: 0.68, wind: -0.2, color: "#D97732", frames: 5, format: "gif" }) });
  assert.equal(leafFall.status, 200); assert.equal((await leafFall.json()).data.operation, "generate_leaf_fall_overlay");
  const ripple = await fetch(`${rest.url}/api/v1/effects/water-ripple`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_filename: "hero-ripple.gif", seed: 109, density: 0.72, amplitude: 3, color: "#BDEBFF", frames: 5, format: "gif" }) });
  assert.equal(ripple.status, 200); assert.equal((await ripple.json()).data.operation, "generate_water_ripple_overlay");
    ...
    const effectStack = await fetch(`${rest.url}/api/v1/effects/scene-stack`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ input_filename: "hero.png", output_prefix: "art/hero", effects: ["material_texture", "rain", "fog", "wind_sway", "sprite_shadow", "sprite_glow", "particles"], frames: 6, seed: 9 }) });
