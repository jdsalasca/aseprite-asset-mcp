...
export interface AssetScenePlanResult { operation: "plan_asset_scene"; libraryVersion: string; itemIds: string[]; layers: Array<{ id: string; assetId: string; title: string; category: string; kind: AssetLibraryItem["kind"]; role: "background" | "midground" | "foreground" | "effect"; order: number; previewPath: string; spritePath: string }>; deterministic: true; sourcePreserved: true; }
