...
export class SpriteEffectsService implements SpriteEffectsGateway {
  ...
  public async generateFogOverlay(input: FogOverlayInput): Promise<AssetOperationResult> {
    try {
      assertDifferent(input.inputFilename, input.outputFilename);
      const color = rgba(input.color);
      const density = input.density ?? 0.55;
      const drift = input.drift ?? 0;
      if (!Number.isInteger(input.seed)) throw new Error("Fog seed must be an integer");
      if (!Number.isFinite(density) || density < 0 || density > 1) throw new Error("Fog density must be between 0 and 1");
      if (!Number.isFinite(drift) || drift < -1 || drift > 1) throw new Error("Fog drift must be between -1 and 1");
      const source = await this.codec.decode(input.inputFilename);
      const frameCount = input.frames ?? source.length;
      if (!Number.isInteger(frameCount) || frameCount < 1 || frameCount > 24) throw new Error("Fog frames must be an integer from 1 to 24");
      if (frameCount > 1 && input.format === "png") throw new Error("Fog animations with more than one frame require GIF format");
      const frames = Array.from({ length: frameCount }, (_, frameIndex) => {
        const sourceFrame = source[frameIndex % source.length];
        if (!sourceFrame) throw new Error("Fog requires at least one source frame");
        const pixels = new Uint8ClampedArray(sourceFrame.pixels);
        const scale = Math.max(3, Math.round(Math.min(sourceFrame.width, sourceFrame.height) * (0.22 + density * 0.18)));
        for (let y = 0; y < sourceFrame.height; y += 1) for (let x = 0; x < sourceFrame.width; x += 1) {
          if (alphaAt(sourceFrame, x, y) === 0) continue;
          const phaseX = (x + drift * frameIndex * sourceFrame.width + input.seed * 0.31) / scale;
          const phaseY = (y - frameIndex * (0.35 + Math.abs(drift)) + input.seed * 0.17) / (scale * 0.8);
          const cloud = (Math.sin(phaseX * 1.7 + Math.sin(phaseY)) + Math.sin(phaseY * 1.35 + Math.cos(phaseX * 0.7))) * 0.25 + 0.5;
          const localDensity = clamp((cloud - 0.16) * 1.25, 0, 1) * density;
          const alpha = Math.round(color[3] * localDensity * (0.24 + hash(input.seed + frameIndex * 53, x + y * sourceFrame.width) * 0.46));
          if (alpha > 0) overlayPixel(pixels, sourceFrame.width, sourceFrame.height, x, y, [color[0], color[1], color[2], alpha], 1);
        }
        const markerX = Math.abs((input.seed * 17 + frameIndex * 5) % sourceFrame.width);
        const markerY = Math.abs((input.seed * 11 + frameIndex * 3) % sourceFrame.height);
        if (density > 0) overlayPixel(pixels, sourceFrame.width, sourceFrame.height, markerX, markerY, [color[0], color[1], color[2], Math.round(color[3] * density * 0.5)], 1);
  ...
  public async generateLightningOverlay(input: LightningOverlayInput): Promise<AssetOperationResult> {
    try {
      assertDifferent(input.inputFilename, input.outputFilename);
      const color = rgba(input.color);
      const intensity = input.intensity ?? 0.65;
      const flash = input.flash ?? 0.65;
      if (!Number.isInteger(input.seed)) throw new Error("Lightning seed must be an integer");
      if (!Number.isFinite(intensity) || intensity < 0 || intensity > 1) throw new Error("Lightning intensity must be between 0 and 1");
      if (!Number.isFinite(flash) || flash < 0 || flash > 1) throw new Error("Lightning flash must be between 0 and 1");
      const source = await this.codec.decode(input.inputFilename);
      const frameCount = input.frames ?? source.length;
      if (!Number.isInteger(frameCount) || frameCount < 1 || frameCount > 24) throw new Error("Lightning frames must be an integer from 1 to 24");
      if (frameCount > 1 && input.format === "png") throw new Error("Lightning animations with more than one frame require GIF format");
      const frames = Array.from({ length: frameCount }, (_, frameIndex) => {
        const sourceFrame = source[frameIndex % source.length];
        if (!sourceFrame) throw new Error("Lightning requires at least one source frame");
        const pixels = new Uint8ClampedArray(sourceFrame.pixels);
        const center = sourceFrame.width * (0.25 + hash(input.seed, frameIndex + 3) * 0.5);
        const slant = (hash(input.seed + 11, frameIndex) - 0.5) * 0.8;
        for (let y = 0; y < sourceFrame.height; y += 1) for (let x = 0; x < sourceFrame.width; x += 1) {
          if (alphaAt(sourceFrame, x, y) === 0) continue;
          const expectedX = center + slant * y + Math.sin(y * 1.7 + frameIndex) * Math.max(1, sourceFrame.width * 0.08);
          const distance = Math.abs(x - expectedX);
          const bolt = clamp(1 - distance / Math.max(1, sourceFrame.width * 0.22), 0, 1);
          const sceneFlash = flash * (0.15 + 0.35 * (0.5 + 0.5 * Math.sin((frameIndex + input.seed) * 1.9)));
          const mix = clamp((sceneFlash + bolt * intensity) * 0.55, 0, 1);
          if (mix > 0.02) overlayPixel(pixels, sourceFrame.width, sourceFrame.height, x, y, [color[0], color[1], color[2], color[3]], mix);
        }
        const markerX = Math.abs((input.seed * 37 + frameIndex * 7) % sourceFrame.width);
        const markerY = Math.abs((input.seed * 23 + frameIndex * 5) % sourceFrame.height);
        if (intensity > 0) overlayPixel(pixels, sourceFrame.width, sourceFrame.height, markerX, markerY, [color[0], color[1], color[2], Math.round(220 * intensity)], 0.35);
        return { ...sourceFrame, pixels, delayMs: input.delayMs ?? sourceFrame.delayMs ?? 90 };
      });
      const format = formatFor(frames, input.format);
      await this.codec.encode(frames, input.outputFilename, format);
  ...
  public async generateWaterSpray(input: WaterSprayInput): Promise<AssetOperationResult> {
    try {
      assertDifferent(input.inputFilename, input.outputFilename);
      const color = rgba(input.color); const density = input.density ?? 0.6; const drift = input.drift ?? 0;
      if (!Number.isInteger(input.seed)) throw new Error("Water spray seed must be an integer");
      if (!Number.isFinite(density) || density < 0 || density > 1) throw new Error("Water spray density must be between 0 and 1");
      if (!Number.isFinite(drift) || drift < -1 || drift > 1) throw new Error("Water spray drift must be between -1 and 1");
      const source = await this.codec.decode(input.inputFilename); const frameCount = input.frames ?? source.length;
      if (!Number.isInteger(frameCount) || frameCount < 1 || frameCount > 24) throw new Error("Water spray frames must be an integer from 1 to 24");
      if (frameCount > 1 && input.format === "png") throw new Error("Water spray animations with more than one frame require GIF format");
      const frames = Array.from({ length: frameCount }, (_, frameIndex) => {
        const sourceFrame = source[frameIndex % source.length]; if (!sourceFrame) throw new Error("Water spray requires at least one source frame");
        const pixels = new Uint8ClampedArray(sourceFrame.pixels); const dropletCount = Math.max(1, Math.round(sourceFrame.width * sourceFrame.height * 0.04 * density));
        for (let droplet = 0; droplet < dropletCount; droplet += 1) {
          const base = input.seed + droplet * 53; const x = Math.floor(hash(base, 3) * sourceFrame.width); const y = Math.floor((0.35 + hash(base, 7) * 0.35) * sourceFrame.height);
          const travel = Math.round(frameIndex * (0.7 + hash(base, 11) * 1.4)); const horizontal = Math.round(drift * frameIndex * (1 + hash(base, 13) * 2)); const size = density > 0.7 && hash(base, 17) > 0.72 ? 2 : 1;
          const alpha = Math.round(color[3] * (0.35 + hash(base, 19) * 0.55) * density);
          for (let offset = 0; offset < size; offset += 1) overlayPixel(pixels, sourceFrame.width, sourceFrame.height, x + horizontal + offset, y - travel + offset, [color[0], color[1], color[2], alpha], 1);
        }
        let markerX = Math.abs((input.seed * 43 + frameIndex * 7) % sourceFrame.width); let markerY = Math.abs((input.seed * 17 + frameIndex * 5) % sourceFrame.height);
        ...
        if (density > 0 && alphaAt(sourceFrame, markerX, markerY) > 0) { const markerAlpha = Math.round(color[3] * density * (0.38 + (frameIndex % 4) * 0.14)); overlayPixel(pixels, sourceFrame.width, sourceFrame.height, markerX, markerY, [color[0], color[1], color[2], markerAlpha], 1); }
        return { ...sourceFrame, pixels, delayMs: input.delayMs ?? sourceFrame.delayMs ?? 90 };
      });
      const format = formatFor(frames, input.format); await this.codec.encode(frames, input.outputFilename, format);
      return outputMessage("generate_water_spray", input.inputFilename, input.outputFilename, frames.length, format);
    } catch (error) { return fail(error); }
  }

  public async generateDustOverlay(input: DustOverlayInput): Promise<AssetOperationResult> {
    try {
      assertDifferent(input.inputFilename, input.outputFilename);
      const color = rgba(input.color); const density = input.density ?? 0.6; const drift = input.drift ?? 0; const rise = input.rise ?? 0.5;
      if (!Number.isInteger(input.seed)) throw new Error("Dust seed must be an integer");
      if (!Number.isFinite(density) || density < 0 || density > 1) throw new Error("Dust density must be between 0 and 1");
      if (!Number.isFinite(drift) || drift < -1 || drift > 1) throw new Error("Dust drift must be between -1 and 1");
      if (!Number.isFinite(rise) || rise < 0 || rise > 1) throw new Error("Dust rise must be between 0 and 1");
      const source = await this.codec.decode(input.inputFilename); const frameCount = input.frames ?? source.length;
      if (!Number.isInteger(frameCount) || frameCount < 1 || frameCount > 24) throw new Error("Dust frames must be an integer from 1 to 24");
      if (frameCount > 1 && input.format === "png") throw new Error("Dust animations with more than one frame require GIF format");
      const frames = Array.from({ length: frameCount }, (_, frameIndex) => {
        const sourceFrame = source[frameIndex % source.length]; if (!sourceFrame) throw new Error("Dust requires at least one source frame");
        const pixels = new Uint8ClampedArray(sourceFrame.pixels); const grainCount = Math.max(1, Math.round(sourceFrame.width * sourceFrame.height * 0.045 * density));
        for (let grain = 0; grain < grainCount; grain += 1) {
          const base = input.seed + grain * 61; const x = Math.floor(hash(base, 3) * sourceFrame.width); const y = Math.floor((0.55 + hash(base, 7) * 0.35) * sourceFrame.height);
          const vertical = Math.round(frameIndex * rise * Math.max(1, sourceFrame.height * 0.25)); const horizontal = Math.round(drift * frameIndex * (1 + hash(base, 11) * 2));
          const wobble = Math.round(Math.sin(frameIndex * 1.4 + hash(base, 13) * 6) * (1 + density * 2)); const size = density > 0.72 && hash(base, 17) > 0.78 ? 2 : 1;
          const alpha = Math.round(color[3] * (0.25 + hash(base, 19) * 0.55) * density);
          for (let offset = 0; offset < size; offset += 1) overlayPixel(pixels, sourceFrame.width, sourceFrame.height, x + horizontal + wobble + offset, y - vertical + offset, [color[0], color[1], color[2], alpha], 1);
        }
        let markerX = Math.abs((input.seed * 47 + frameIndex * 7) % sourceFrame.width); let markerY = Math.abs((input.seed * 23 + frameIndex * 5) % sourceFrame.height);
        if (alphaAt(sourceFrame, markerX, markerY) === 0) outer: for (let candidateY = 0; candidateY < sourceFrame.height; candidateY += 1) for (let candidateX = 0; candidateX < sourceFrame.width; candidateX += 1) if (alphaAt(sourceFrame, candidateX, candidateY) > 0) { markerX = candidateX; markerY = candidateY; break outer; }
        if (density > 0 && alphaAt(sourceFrame, markerX, markerY) > 0) { const markerAlpha = Math.round(color[3] * density * (0.32 + (frameIndex % 4) * 0.15)); overlayPixel(pixels, sourceFrame.width, sourceFrame.height, markerX, markerY, [color[0], color[1], color[2], markerAlpha], 1); }
        return { ...sourceFrame, pixels, delayMs: input.delayMs ?? sourceFrame.delayMs ?? 90 };
      });
      const format = formatFor(frames, input.format); await this.codec.encode(frames, input.outputFilename, format);
