...
import type { AssetSceneAnimationComposerService } from "../../application/services/AssetSceneAnimationComposerService.js";

export class AssetLibraryToolController {
  public constructor(private readonly library: AssetLibraryService, private readonly auditService: AssetLibraryAuditService, private readonly summaryService: AssetLibrarySummaryService, private readonly scenePlanner: AssetScenePlannerService, private readonly sceneComposer: AssetSceneComposerService, private readonly sceneAnimationComposer: AssetSceneAnimationComposerService) {}
  ...
  public register(server: McpServer): void {
    ...
    server.registerTool("compose_asset_scene_animation", { description: "Compose selected animated library previews into a deterministic GIF and frame manifest.", inputSchema: { item_ids: z.array(z.string().min(1)).min(1).max(24), output_filename: z.string().min(1), manifest_filename: z.string().min(1), width: z.number().int().min(16).max(2048), height: z.number().int().min(16).max(2048), padding: z.number().int().min(0).max(64).default(0), frames: z.number().int().min(2).max(24).default(8), delay_ms: z.number().int().min(1).max(2000).default(90) } }, async ({ item_ids, output_filename, manifest_filename, width, height, padding, frames, delay_ms }) => this.result(await this.sceneAnimationComposer.compose({ itemIds: item_ids, outputFilename: output_filename, manifestFilename: manifest_filename, width, height, padding, frames, delayMs: delay_ms })));
  }
