...
export class AssetRestController {
  ...
  public async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/v1/library") return this.send(response, 200, { data: await this.useCases.assetLibrary.search({ ...(url.searchParams.get("query") ? { query: url.searchParams.get("query")! } : {}), ...(url.searchParams.get("category") ? { category: url.searchParams.get("category")! } : {}), ...(url.searchParams.has("limit") ? { limit: Number(url.searchParams.get("limit")) } : {}) }) }, origin);
      if (request.method === "GET" && url.pathname.startsWith("/api/v1/library/items/")) { const item = await this.useCases.assetLibrary.get(decodeURIComponent(url.pathname.slice("/api/v1/library/items/".length))); return item ? this.send(response, 200, { data: item }, origin) : this.send(response, 404, { error: "Asset library item not found" }, origin); }
      if (request.method === "GET" && url.pathname.startsWith("/api/v1/library/presets/")) { const preset = await this.useCases.assetLibrary.preset(decodeURIComponent(url.pathname.slice("/api/v1/library/presets/".length))); return preset ? this.send(response, 200, { data: preset }, origin) : this.send(response, 404, { error: "Asset library preset not found" }, origin); }
  ...
  private async readBody(request: IncomingMessage): Promise<unknown> {
    let raw = "";
    let size = 0;
    for await (const chunk of request) { const piece = chunk.toString(); size += Buffer.byteLength(piece); if (size > MAX_BODY_BYTES) throw new Error("Request body exceeds 1 MiB limit"); raw += piece; }
